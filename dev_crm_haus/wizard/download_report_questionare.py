...
class DownloadReportQuestionare(models.TransientModel):
    ...
    start_date_submited_date_fields = fields.Date(string='Start Date Submitted')
    end_date_submited_date_fields = fields.Date(string='End Date Submitted')
    list_questionare_fields = fields.Many2one('crm.questionare.admin',string="Questionare")
    questionare_name_fields = fields.Char(related='list_questionare_fields.questionare_name_fields')
    ...
    name_assigned_user_fields = fields.Many2one('employee.data',string="Assined Users",domain = "[('current_status_employee','=','Active')]")
    report_files = fields.Binary(string='Report Data',compute='download_report_completed')
    ...
    def download_report_completed(self):
        ...
        if search_data:
            ...
            for data in search_data:
                ...
                search_user = self.env['employee.data'].search([('email_employee','=',data.email_employee)])
...
class DownloadReportQuestionareMissed(models.TransientModel):
    ...
    list_questionare_fields = fields.Many2one('crm.questionare.admin',string="Questionare")
    questionare_name_fields = fields.Char(related='list_questionare_fields.questionare_name_fields')
    name_assigned_user_fields = fields.Many2one('employee.data',string="Assigned User",domain = "[('current_status_employee','=','Active')]")
    report_files = fields.Binary(string='Report Data',compute='download_report_missed')
...
class DownloadReportIssue(models.TransientModel):
    ...
    temporary_location_selection_fields = fields.Selection(site_list,string="Sites Selection", default="Haus Office Meruya")
    start_date_created_date_issue_fields = fields.Date(string='Start Date Created Issue')
    end_date_created_date_issue_fields = fields.Date(string='End Date Created Issue')

    start_date_solved_date_issue_fields = fields.Date(string='Start Date Solved Issue')
    end_date_solved_date_issue_fields = fields.Date(string='End Date Solved Issue')
    ...
    reporter_employee = fields.Many2one('employee.data',string="Reporter",domain = "[('current_status_employee','=','Active')]")
    assigned_employee = fields.Many2one('employee.data',string="Assigned Users",domain = "[('current_status_employee','=','Active')]")
    issue_problem = fields.Many2one('crm.issue')
    issue_problem_name = fields.Char(related='issue_problem.issue_problem')
    report_files = fields.Binary(string='Report Data',compute='download_report')
